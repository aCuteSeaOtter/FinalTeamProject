	        	...
	        	<form action="/selectMyPlan" method="post" id="myPlanForm">
	        		<c:forEach items="${planList}" var="list">
	        			<div class="wrapper">
	        				<div class="block">
	        					<div class="left">
	        						<div>일정명 : ${list.INFO_NAME}</div>
		        					<div>작성자 : ${list.MEMBER_EMAIL}</div>
		        					<div>지역 : ${list.TRIP_PLACE}</div>
	        					</div>
	        					<div class="right">
	        						<div>시작일 : ${list.START_DATE}</div>
	        						<div>종료일 : ${list.END_DATE}</div>
	        					</div>
	        					<input type="text" name="info_id" class="info_id" value="${list.INFO_ID}">
	        				</div>
	        			</div>
	        		</c:forEach>
	        	</form>
